namespace auto_Bot_603;
using ChessChallenge.API;
using System;
using System.Collections.Generic;
using System.Linq;

public class Bot_603 : IChessBot
{
    private Timer timer;
    ...
    int AlphaBeta(Board board, int alpha, int beta, int depth)
    {
        ...
        if (depth == 0)
        {
            return Quiesce(board, alpha, beta, false);
        }
    ...
    int Quiesce(Board board, int alpha, int beta, bool isPromotion)
    {
        if (board.IsInCheckmate())
        {
            return -100000;
        ...
        foreach (Move move in moves)
        {
            ...
            board.MakeMove(move);
            int eval = -Quiesce(board, -beta, -alpha, move.IsPromotion);
            board.UndoMove(move);
