                                    ...
                                    class Bot_266 : IChessBot
{
    ...
    int EB()
    {
        float e = eg
                           ...
                           ; return (int)(bo.IsWhiteToMove ? s : -s);
    }
    ulong T(ulong[]
                          [] t, PieceList pl)
    {
        ulong v = 0; foreach (var p in pl)
        {
            var sq =
                        p.Square; v += t[(int)p.PieceType][sq.File >= 0x4 ? 07 - sq.File : sq.
                       File] << 8 * (pl.IsWhitePieceList ? 07 - sq.Rank : sq.Rank) >> 56;
        }
        return
                       25 * v;
    }
    (int, Move, bool) SE(int dL, int cL, bool co, int a, int b)
    {
        if (
        ...
        {
            ++dL; if (bo.IsInCheck() && cL > 0) --cL;
            else if (!co && cL == 4)
                return SE(0x08, cL, true, a, b);
            else return (EB(), default, true);
        }
        ulong
                k = bo.ZobristKey; E ta = tr[k % 16_7_7_7_2_16]; int bS = -32150, s; Move be =
                 default; if (ta.K == k && Abs(ta.D) >= dL)
        {
            bo.MakeMove(ta.M); bool tD = bo.
                 IsDraw(); bo.UndoMove(ta.M); if (tD) ta = default;
            else
            {
                a = Max(a, bS = ta.
                  S); be = ta.M; if (b < a || ta.D >= 0) return (ta.S, ta.M, true);
            }
        }
        if (co && (s
             ...
             Item1)); bool cT = true, ax = false, cU; lv = 0; foreach (var (_, m) in prioms)
        {
            if (o = t
           .MillisecondsElapsedThisTurn >= this.b) return (bS, be, cT); bo.MakeMove(m); try
            {
                if (
         dL >= 3 && ++lv >= 4 && !m.IsCapture)
                {
                    s = -SE(dL - 2, cL, co, -b, -a).Item1; if (o) break; if (s < bS)
                        continue;
                }
                (s, _, cU) = SE(dL - 01, cL, co, -b, -a); if (o) break; s = -s + (Abs(s) >= 30000 ? Sign(s) : 0);
                if (s <= bS) continue; bS = s; be = m; a = Max(a, s); cT = cU; if (ax = b < a) { km.Add(m); break; }
            }
            finally
            {
                bo.
        ...
        if (!o && !co && cT && bS != 0) tr[k % 16777216] = new E
        {
            K = k,
            D = (short)(ax ? -dL : dL),
            S = (short)
    ...
    public Move Think(Board c, Timer ti)
    {
        bo = c; t = ti; b = Min(0.0333333333f,
...
0x0c0f0e0d0c0b0a06ul,0x0e100f0e0d0c0b0aul,0x0e1114171614100aul,0x0e1116191815100aul}}; struct E
    {
        public ulong
K; public short S, D; public Move M;
    }
} /* Thank you for hosting this competition, Sebastian!  <3 <3 <3 <3  */